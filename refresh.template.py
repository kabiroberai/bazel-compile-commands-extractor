...
def _warn_if_file_doesnt_exist(source_file):
    if not os.path.isfile(source_file):
        # Just log once; subsequent messages wouldn't add anything.
        if not _warn_if_file_doesnt_exist.has_logged_missing_file_error:
...
def _get_files(compile_action):
    ...
    if len(source_file_candidates) > 1:
        ...
        assert source_file.endswith(_get_files.clang_source_extensions), f"Source file candidate, {source_file}, seems to be wrong.\nSelected from {compile_action.arguments}.\nPlease file an issue with this information!"
...
_get_files.clang_source_extensions = _get_files.c_source_extensions + _get_files.cpp_source_extensions + _get_files.objc_source_extensions + _get_files.objcpp_source_extensions + _get_files.cuda_source_extensions + _get_files.opencl_source_extensions + _get_files.openclxx_source_extensions + _get_files.assembly_source_extensions + _get_files.assembly_needing_c_preprocessor_source_extensions
...
def _get_commands(target: str, flags: str):
    ...
    aquery_args = [
        ...
        f"mnemonic('(Objc|Cpp|Cuda|Swift)Compile',{target_statment})",
